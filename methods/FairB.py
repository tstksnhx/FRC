...
def expm(seed=1, alpha=0., data='adult'):
    ...
    if data == 'heritage':
        ...
        print('train', x.shape, y.shape, a.shape)
...
if __name__ == '__main__':
    ...
    #
    # for s in range(10):
    #     expm(seed=s, alpha=0, data='compas-M')

    ps = '../npz_data/compas_s_1.npz'
    data_d = tools.load_single(ps)
    train = data_d['train']
    verify = data_d['verify']
    test = data_d['test']
    x, y, s = test
    x = x.numpy()
    y = y.numpy()
    s = s.numpy()
    np.savez("compas_test", x=x,
             y=y,
             sensitives=s)
